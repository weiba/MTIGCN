...
from myutils import *
from Internal.New.MTIGCN_New import mulgcn_new
...
for dim in target_dim:
    for target_index in np.arange(res.shape[dim]):
        ...
        for fold in range(n_kfold):
            true_data, predict_data = mulgcn_new(cell_sim=cell_sim,drug_sim=drug_sim,cell_exprs=exprs,drug_finger=drug_finger,res_ic50 = res_ic50,
        ...
        if dim:
            true_data_s.to_csv("./result_mul/{}_drug_".format(args.data) + str(target_index) + "_true_data.csv")
            predict_data_s.to_csv("./result_mul/{}_drug_".format(args.data) + str(target_index) + "_predict_data.csv")
        else:
            true_data_s.to_csv("./result_mul/{}_cell_".format(args.data) + str(target_index) + "_true_data.csv")
            predict_data_s.to_csv("./result_mul/{}_cell_".format(args.data) + str(target_index) + "_predict_data.csv")
