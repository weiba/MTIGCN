...
for para in paras:
    ...
    for n_kfold in range(n_kfolds):
        for train_index, test_index in kfold.split(np.arange(pos_num)):
            ...
            opt = Optimizer_mul(model, args.loss_ic50,args.loss_c,args.loss_d,args.loss_binary,cell_sim, drug_sim, sampler.train_ic50, sampler.test_ic50, sampler.train_data, sampler.test_data, sampler.test_mask, sampler.train_mask,
    ...
    pd.DataFrame(true_datas).to_csv("./result_data/{}_1true_data.csv".format(args.data))
    pd.DataFrame(predict_datas).to_csv("./result_data/{}_1predict_data.csv".format(args.data))
