...
def _load_gdsc(args):
    ...
    #res_ic50 = res_ic50.replace(np.nan, 0)
    # Default plot
...
def _load_pdx(args):
    ...
    # 加载细胞系-药物IC50反应矩阵
    gdsc_ic50 = pd.read_csv(gdsc_data_dir + "cell_drug.csv", index_col=0, header=0)
    gdsc_ic50 = np.array(gdsc_ic50, dtype=np.float32)
    ...
    pdx_ic50 = np.zeros(shape=pdx_res.shape, dtype=np.float32)
    # 合并GDSC-PDX反应矩阵
    res = np.concatenate((gdsc_res, pdx_res), axis=0)
    train_row = gdsc_res.shape[0]
    res_ic50 = np.concatenate((gdsc_ic50, pdx_ic50), axis=0)
    ...
    return res_ic50, res, drug_feature, exprs, null_mask, train_row, args
...
def _load_tcga(args):
    ...
    tcga_ic50 = np.zeros(shape=tcga_res.shape, dtype=np.float32)
    # 合并GDSC-TCGA反应矩阵
    res = np.concatenate((gdsc_res, tcga_res), axis=0)
    res_ic50 = np.concatenate((gdsc_ic50, tcga_ic50), axis=0)
    train_row = gdsc_res.shape[0]
...
def _load_unknown(args):
    ...
    gdsc_null_mask = pd.read_csv(gdsc_data_dir + "null_mask.csv", index_col=0, header=0)
    gdsc_null_mask = np.array(gdsc_null_mask, dtype=np.float32)
    res_num = np.sum(gdsc_null_mask,axis=0)
    all_num = np.sum(res_num)
    print(all_num)
    print(res_num)
    print(np.argmax(res_num))

    return gdsc_ic50, gdsc_res, drug_feature, gdsc_res, gdsc_null_mask, train_row, args
