...
def distribute_compute(lr_list,wd_list,scale_list,layer_size,workers:int,id:int):
    all_list = []
    for lr,wd,sc,la, in it.product(lr_list,wd_list,scale_list,layer_size):
        all_list.append([lr,wd,sc,la])
...
class EarlyStop(object):
    def __init__(self, tolerance: int, data_len: int, dtype=torch.float32, device="cpu"):
        self.tolerance = tolerance
        self.len = 0
        self.auc_pre = None
        self.epoch_pre = None
        self.predict_data_pre = None
        self.auc = torch.zeros(tolerance, dtype=dtype, device=device)
        self.epoch = torch.zeros(tolerance, dtype=dtype, device=device)
        self.predict_data = torch.zeros([tolerance, data_len], dtype=dtype, device=device)
    ...
    def average(self):
        if self.len < self.tolerance:
            ...
            avg = torch.mean(self.auc)
        return avg

    def stop(self, auc, epoch, predict_data):
        avg_pre = self.average()
        self.auc_pre = self.auc.clone()
        self.epoch_pre = self.epoch.clone()
        self.predict_data_pre = self.predict_data.clone()
        self.push_data(auc=auc, epoch=epoch, predict_data=predict_data)
        avg_next = self.average()
        flag = False
        if avg_pre > avg_next:
            flag = True
        return flag
    ...
    def get_best_index(self):
        best_index = torch.argmax(self.auc_pre)
        if self.epoch_pre[best_index] == 0:
            self.auc_pre[best_index] = 0
            best_index = torch.argmax(self.auc_pre)
        return best_index.item()
...
def cross_entropy_loss(true_data: torch.Tensor, predict_data:  torch.Tensor, masked: torch.Tensor):
    ...
    true_data = torch.masked_select(true_data, masked)
    predict_data = torch.masked_select(predict_data, masked)
    # weight = torch.zeros_like(true_data).float().cuda()
    # weight = torch.fill_(weight, 0.3)
    # weight[true_data > 0] = 0.7
    # loss_fun = nn.BCELoss(weight=weight)
    loss_fun = nn.BCELoss()
    loss = loss_fun(predict_data, true_data)
    return loss
...
def pcc(true_data: torch.Tensor, predict_data: torch.Tensor):
    """
    :param true_data: train data, torch tensor 1D
    :param predict_data: predict data, torch tensor 1D
    :return: pcc score
