...
class RandomSampler_ic50(object):
    # 对原始边进行采样
    # 采样后生成测试集、训练集
    # 处理完后的训练集转换为torch.tensor格式

    def __init__(self, adj_ic50, adj_binary, train_index, test_index, null_mask):
        super(RandomSampler_ic50, self).__init__()
...
class NewSampler(object):
    ...
    def sample_train_test_mask(self):
        ...
        neg_test_mask = np.zeros(self.adj_mat.shape, dtype=np.float32)
        if self.dim:
            neg_test_mask[:, self.target_index] = 1
            neg_value[:, self.target_index] = 0
        else:
            neg_test_mask[self.target_index, :] = 1
            neg_value[self.target_index, :] = 0
...
class NewSampler_ic50(object):
    def __init__(self, adj_ic50, original_adj_mat, null_mask, target_dim, target_index):
        super(NewSampler_ic50, self).__init__()
        self.adj_ic50 = torch.tensor(np.nan_to_num(adj_ic50))
        self.adj_mat = original_adj_mat
        self.null_mask = null_mask
        self.dim = target_dim
...
class SingleSampler_ic50(object):
    """
    对指定目标进行采样
    采样返回结果为torch.tensor
    """
    def __init__(self, adj_ic50, origin_adj_mat, null_mask, target_index, train_index, test_index):
        super(SingleSampler_ic50, self).__init__()
        self.adj_ic50 = torch.tensor(np.nan_to_num(adj_ic50))
        self.adj_mat = origin_adj_mat
        self.null_mask = null_mask
        self.target_index = target_index
...
class TargetNewSampler(object):
    def __init__(self, original_adj_mat, null_mask, target_dim, target_index):
        super(TargetNewSampler, self).__init__()
        self.adj_mat = original_adj_mat
        self.null_mask = null_mask
        self.dim = target_dim
        self.target_index = target_index
...
class ExterSampler_ic50(object):
    def __init__(self, adj_ic50, original_adj_mat, null_mask, train_index, test_index):
        super(ExterSampler_ic50, self).__init__()
        self.adj_ic50 = torch.tensor(np.nan_to_num(adj_ic50))
        self.adj_mat = original_adj_mat
        self.null_mask = null_mask
        self.train_index = train_index
...
class CaseSampler(object):
    def __init__(self, adj_ic50, original_adj_mat, null_mask, train_index):
        super(CaseSampler, self).__init__()
        self.adj_ic50 = torch.tensor(np.nan_to_num(adj_ic50))
        self.adj_mat = original_adj_mat
        self.null_mask = null_mask
        self.train_index = train_index
        self.train_data = self.sample_train_data()
        self.train_mask, self.test_mask = self.sample_train_test_mask()
        self.train_ic50 = torch.mul(self.train_mask, self.adj_ic50)
        self.test_ic50 = torch.mul(self.test_mask, self.adj_ic50)

    def sample_train_data(self):
        train_data = self.adj_mat
        train_data = torch.from_numpy(train_data)
        return train_data
    ...
    def sample_train_test_mask(self):
        ...
        neg_train = neg_value - self.adj_mat - self.null_mask
        train_mask = (self.train_data.numpy() + neg_train).astype(np.bool)
        test_mask = (self.null_mask).astype(np.bool)
        train_mask = torch.from_numpy(train_mask)
        test_mask = torch.from_numpy(test_mask)
        return train_mask, test_mask